...
for (i = 0; i<res.length;i++){
  ...
  $time = $time[0]+':'+$time[1];
  if (res[i]['price']==0){
    $status='FREE<br> <button type="button" class="btn btn-danger" class="control" id="cancelEvent" onclick="CancelEvent('+res[i]["id"]+')">Cancel</button>';
  }
  else if (res[i]['status_id']=='W'){
